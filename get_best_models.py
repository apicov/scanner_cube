...
dest_dir  = "/home/pico/uni/romi/scanner_cube/volumes/"
if not os.path.exists(dest_dir):
    os.makedirs(dest_dir)


f = open(os.path.join(dest_dir,'voxel_count.csv'),'w')
writer = csv.writer(f)
...
for model in range(200):
    ...
    writer.writerow( [str(model).zfill(3),h[0],h[1],h[2]])
    np.save(os.path.join(dest_dir,str(model).zfill(3)+'_vol'), spc.sc.values())
